...
const CurrencyCard: React.FC<CurrencyCardProps> = ({
  ...
  return (
    <>
      <h2 className="text-2xl font-bold mb-2">{currency} Exchange Rates</h2>
      <div className="flex justify-between items-start">
        ...
        <div className="bg-green-100 rounded-lg flex flex-col items-center flex-1">
          ...
          <span className="mt-2 text-xl font-bold">{buyRate.toFixed(4)}</span>
        </div>
        {/* Sell Box */}
        <div className="bg-red-100 rounded-lg flex flex-col items-center flex-1">
          <span className="text-lg font-semibold">Sell</span>
          <span className="mt-2 text-xl font-bold">{sellRate.toFixed(4)}</span>
        </div>
      </div>
    </>
  );
